...
import { prodansvarlige, Flexer } from './common/teammedlemmer'

console.log('🚀 Starter generer-prod-ansvar.ts')
...
if (ukeInput) {
    ...
    if (!isNaN(ukeArg) && ukeArg >= 1 && ukeArg <= 53) {
        ...
        startDato = gjeldende
        console.log(`✅  Genererer prod-ansvar fra uke ${startUke} (${gjeldende.format('YYYY-MM-DD')})`)
    } else {
...
if (personInput) {
    ...
    startPerson = prodansvarlige.find((p) => p.initialer === personInitialer)

    if (startPerson) {
        console.log(`✅  ${startPerson.initialer} starter som prod-ansvarlig fra uke ${startUke}`)
    } else {
        console.log(`❌ Ukjente initialer: ${personInitialer}`)
        console.log('📋 Tilgjengelige prod-ansvarlige:')
        prodansvarlige.forEach((p) => console.log(`  - ${p.initialer}`))
        process.exit(1)
    }
} else {
